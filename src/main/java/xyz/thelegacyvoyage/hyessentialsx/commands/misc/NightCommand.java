...
public final class NightCommand extends CommandBase {

    private static final String PERMISSION_NODE = "hyessentialsx.time.night";
    private static final double NIGHT_TIME = 0.90d;
    ...
    protected void executeSync(@Nonnull CommandContext context) {
        ...
        if (!WorldTimeUtil.setDayTimeFraction(world, NIGHT_TIME)) {
            boolean ran = tryRunTimeCommand(player, "time set night");
            if (ran || WorldTimeUtil.isNight(world)) {
                Messages.okKey(context, "time.night", Map.of());
                return;
            }
            Messages.errKey(context, "time.not_supported", Map.of());
            return;
